...
const CURATED: HideSpot[] = [
  ...
  spot(-3.6, FLOOR, -8.3, 0.5), // 薪カゴと窓クッションの間
  ...
  spot(1.75, 0.3, -8.5, 0.4, (-6 * Math.PI) / 180), // TV台の天面（TVの隣・しれっと。実高さは接地スナップで吸着）
  spot(-4.65, 0.55, 8.3, 0.4, (172 * Math.PI) / 180), // ラダーシェルフの棚段（小物と一緒に）
  spot(8.4, 1.55, -2.6, 0.4, -Math.PI / 2), // 本棚の棚の中（本と並んで座る）
  ...
  spot(-8.83, 3.1, 5.4, 0.4, Math.PI / 2), // 額絵の上（-x壁・フレームのふち）
  spot(-6.9, 3.27, -8.85, 0.4), // 暖炉上の額絵の上
  spot(2.6, 3.18, -8.86, 0.4), // TV上の額絵の上
  spot(8.85, 3.2, 0.6, 0.4, -Math.PI / 2), // 壁掛け時計の上
  spot(5.6, 3.3, 8.85, 0.4, Math.PI), // マクラメのドウェル（横棒）の上
  spot(0.0, 3.58, 8.92, 0.4, Math.PI), // 壁の額（プロシージャル）の上
];
...
function wallRing(): HideSpot[] {
  ...
  for (const [x, z] of pts) {
    ...
    if (hit) continue;
    // R31（契約29）: 何もない開けた壁沿いは「余裕で見つかる」ため除外。家具の 1.0m 以内
