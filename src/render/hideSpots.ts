...
const FLOOR = -0.45; // 「見えている床」＝影受け円盤の面（契約32。部屋の躯体床-0.5はこの5cm下）
...
const CURATED: HideSpot[] = [
  ...
  spot(-4.7, 1.55, 8.3, 0.32, (172 * Math.PI) / 180), // ラダーシェルフの棚段4
  ...
  spot(-4.6, 1.95, 8.3, 0.32, (172 * Math.PI) / 180), // ラダーシェルフの棚段5
  ...
  // ---- R35（契約32）: 高所分布 — 「見上げないと見つからない」層（監査で削除した小さすぎ6点の代替）----
  spot(-3.55, 4.45, -8.75, 0.4), // 窓上の掛棚（左）
  spot(-2.45, 4.45, -8.75, 0.4), // 窓上の掛棚（右）
  spot(2.15, 3.9, -8.7, 0.4), // TV上の高所掛棚（左）
  spot(3.05, 3.9, -8.7, 0.4), // TV上の高所掛棚（右）
  spot(2.95, 4.05, 8.75, 0.4, Math.PI), // +z壁の高所掛棚（左）
  spot(3.85, 4.05, 8.75, 0.4, Math.PI), // +z壁の高所掛棚（右）
  spot(-8.7, 3.75, -1.35, 0.4, Math.PI / 2), // -x壁の高所掛棚（左）
  spot(-8.7, 3.75, -0.45, 0.4, Math.PI / 2), // -x壁の高所掛棚（右）
  spot(0.55, 4.75, -5.0, 0.4), // シャンデリアのリングの上（照明の上・見上げ枠）
  spot(-0.55, 4.7, -5.0, 0.4, Math.PI), // シャンデリアのリングの上（反対側）
  spot(-4.25, 4.84, -8.8, 0.4), // 窓の上枠の上（左端）
  spot(-1.75, 4.84, -8.8, 0.4), // 窓の上枠の上（右端）
...
/** 家具の実測AABB（scene の接地影生成用・契約32）。 */
export const FURNITURE_BOXES = BOXES;
