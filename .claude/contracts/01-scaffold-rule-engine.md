...
- 【実装者追補（メイン転記 2026-08-09）】E6: シードRNG同一シードで配札完全一致（決定性）/ E7: deriveViewFor が他家手札・山札内容を露出しない / E8: 不正アクション拒否時に入力stateが不変 / E9: グループ4枚上限・スート重複拒否 / E10: メルド・付け札での出し切り上がり（捨て札なし上がり）2経路
- 【実装者ノート（メイン転記）】「複数ポン競合」は52枚単デッキでは同一ランク4枚制約により実局面で発生不能のため、優先順リゾルバは防御的実装+人工局面テストのみ。実装DECISION: 状態フィールド名は claimWindow（DOMゲート正規表現との衝突回避）/ 単独7の方向確定=種類確定と解釈 / applyAction は structuredClone で純粋性担保
